...
Θετικό του συγκεκριμένου module είναι ότι όλοι οι παραπάνω αισθητήρες είναι συ\-νδε\-δε\-μέ\-νοι σε ένα κοινό bus Inter-Integrated Circuit (\Abbr{I2C}) \cite{I2C-protocol}, μέσω του οποίου μπορεί πολύ εύκολα να γίνει  η διασύνδεση με την επεξεργαστική μονάδα που επιθυμούμε και να πραγματοποιηθεί επικοινωνία. 
...
Αφού είχαν γίνει οι παραπάνω απαραίτητες ενέργειες - έχοντας πλέον ένα λειτουργικό περιβάλλον - ακολούθησε η διαδικασία πραγματοποίησης επιτυχημένης επικοινωνίας με το κάθε υποσύστημα.
...
Αφού γίνουν τα παραπάνω, συνδέοντας κατάλληλα τα RX - TX του \Abbr{GPS} στο Ra\-spbe\-rry, είναι εφικτό εκτελώντας την εντολή \textbf{cat /dev/ttyS0} να έχουμε πρόσβαση στα πακέτα NMEA που στέλνει το \Abbr{GPS}, που έχουν μορφή παρόμοια με το \List{serial-output}.
...
Ενώ προκειμένου το \Abbr{ROS} να χρησιμοποιεί το \Abbr{GPS}, χρειάστηκε να εγκατασταθεί το πακέτο \textbf{nmea\_navsat\_driver} \cite{nmea-navsat-driver} και παράδειγμα χρήσης με το \Abbr{ROS} υπάρχει στο \List{gps-ros-sample-usage}.  
...
Σε αντίθεση με την σειριακή επικοινωνία, όπως αναφέρθηκε στο \Sect{imu} ο \Abbr{IMU} χρησιμοποιεί το \Abbr{I2C} πρωτόκολλο.
