...
Στο \Fig{Localization-Systems-components} δίνεται η απεικόνιση που έδωσαν οι συγγραφείς του 
\cite{wsn-Localization-systems} για να εξηγήσουν το παραπάνω. Ενώ στο \Fig{Localization-system}
...
\begin{figure} [H]
	...
	\decoRule
	\caption[Localization System overview (RF-based approach)]{Localization System overview (\Abbr{RF}-based approach)}
...
Ενώ το \Fig{Ideal-RSSI-over-distance} περιγράφει σχηματικά το ιδανικό μοντέλο της εξάρτηση της ισχύς με την αύξηση
...
% 	\decoRule
% 	\caption[]{Ideal RSSI over distance based on \cite{ideal-rssi-model} (\href{https://www.semanticscholar.org/paper/Adaptive-Distance-Estimation-Based-on-RSSI-in-802-.-Botta-Simek/8b7669850aecaa7b4f8a1b774f260bff24b20ef6}{URL})}
...
χρόνο \emph{t} για να μεταδοθεί από το ένα node στο άλλο. Στο \Fig{Time-of-Arrival-cases} (a) απεικονίζεται
...
το free node θα πρέπει να βρίσκεται κάπου πάνω σε μία υπερβολική καμπύλη όπως παρουσιάζεται από το \Fig{TDoA-hyberbolas}
...
μπορούμε να συμπεράνουμε όμως τον γεωμετρικό τόπο στον οποίο βρίσκεται το free node. Στο \Sect{Multilateration} - και σε επέκταση αυτού -
...
χρησιμοποιεί multiple signals με single receiver και παρουσιάζεται στο \Fig{Time-Difference-of-Arrival-cases} (b). 
...
Το pattern αυτού του τύπου κε\-ραί\-ας, παρουσιάζεται στο \Fig{Typical-anisotropic-antenna-pattern}
...
σχέσης \EqNum{aoa-equation} \cite{wsn-Localization-techniques} \cite{youtube-phase-difference-equation}. Παράδειγμα αυτού\udot βρίσκεται στο \Fig{Antenna-Arrays} (a).
...
παράδειγμα μίας εξ' αυτών βρίσκεται στο \Fig{Antenna-Arrays} (b).
...
απεικονίζεται στο \Fig{Trilateration-examples} (a) \cite{RSSI-trilateration-Range_based}.
...
των τριών κύκλων, σχηματικά αυτό παρουσιάζεται στο \Fig{Trilateration-examples} (b) και σε αυτήν την περίπτωση καταλήγουμε σε ένα
...
Παράδειγμα αυτής της μεθόδου βρίσκεται στο \Fig{Bounding-Box-example}.
...
γνώση που έχουμε για γωνίες και όχι εκτίμηση της απόστασης. Ένα απλούστερο παράδειγμα παρουσιάζεται στο \Fig{Principles-of-Triangulation}, όπου σε αυτό το παράδειγμα από την τριγωνομετρία,
...
% IMAGE
\FigCaptLabelBasedURL{../Photos/Tringulation-principle.png}{Principles of Triangulation}{Principles-of-Triangulation}
...
από τα ιδανικά μοντέλα. Συνεπώς το \Fig{Triangulation-examples} (c) παρουσιάζει σε πιο ρεαλιστικά επίπεδα - συνυπολογίζοντας την ύπαρξη 
των αποκλίσεων - 
...
στην βιβλιογραφία \cite{multilateration-def} \cite{triangulation-trilateration-multilateration} \cite{wikipedia-multilateration}. Στο \Sect{Distance-Angle-Estimation} 
...
Παράδειγμα αυτής της μεθόδου βρίσκεται στο \Fig{Multilateration}.

% IMAGE
\FigCaptLabelBasedURL{../Photos/multilateration.png}{Multilateration example}{Multilateration}
...
% IMAGE
\FigCaptLabelBasedURL{../Photos/localization-graph.png}{Graph example}{Graph-example}
...
Στο \Sect{Energy-based} αναλύθηκαν τεχνικές εκτίμησης απόστασης ή γωνίας μεταξύ των nodes - συγκεκριμένα οι
