...
των οποίων τελικά για δεδομένες θέσεις των beacon - γνωστά δηλαδή τα $(x_i,y_i)$ για κάθε beacon i - και με βάση την μέτρηση των γωνιών
να υπολογίσουμε την θέση του free node $(x,y)$.

\begin{equation}
	\begin{gathered}
		x = \frac{y_2 - y_1-  x_2\tan \phi_2 + x_1\tan \phi_1}{\tan \phi_1 - \tan \phi_2} \\[2pt]
...
Η μέθοδος του Multilateration\footnote{Κάποιες φορές ο όρος αυτός χρησιμοποιείται επίσης σε αναφορά της χρήσης του Trilateration με πάνω από 3 beacons \cite{wsn-Localization-systems}.}, 
...
\begin{table}[H]
    \caption{Distance/angle estimation techniques and position Computation overview}
    \label{tab:Distance/angle-estimation-techniques-and-position-Computation-overview}
	\centering
	\resizebox{1\textwidth}{!}{
		\begin{tabular}{ccccc}
			\toprule
			...
			\midrule
				\hyperref[abbr:RSSI]{RSSI} & \Centerstack{Trila/Multilteration \\ Bounding box} & \Centerstack{Simple \\ No need for extra hardware} & \Centerstack{a} & \Centerstack{a} \\ \hline 
				\hyperref[abbr:ToA]{ToA} & \Centerstack{Trila/Multilteration \\ Bounding box} & \Centerstack{a} & \Centerstack{a} & \Centerstack{a} \\ \hline
				\hyperref[abbr:TDoA]{TDoA} & Multilateration & \Centerstack{a} & Need for extra hardware & \Centerstack{a} \\ \hline
				\hyperref[abbr:AoA]{AoA} & Triangulation & \Centerstack{a} & \Centerstack{a} & \Centerstack{a} \\ \hline
			\bottomrule
		\end{tabular}
	}
\end{table}
