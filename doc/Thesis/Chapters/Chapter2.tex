...
Αφού έχουμε δει τα παραπάνω, ας ορίσουμε το localization problem με ένα πιο αυστηρό μαθηματικό φορμαλισμό.
Έστω ότι έχουμε στην διάθεση μας \emph{n} αριθμό από nodes, και για λόγους απλότητας συμμετρικά και όμοια δίκτυα
επικοινωνίας εμβέλεια \emph{r} για κάθε node. Συνεπώς ένα node \emph{u} επικοινωνεί με ένα node \emph{v} αν και μόνο αν, και το
\emph{v} μπορεί να επικοινωνήσει πίσω στο \emph{u}. Τα οποία είναι διασκορπισμένα σε ένα δισδιάστατο τετραγωνικό
πεδίο \emph{$Q=[0,s]x[0,s]$}.  
...
είναι να γίνει τοπικά η επεξεργασία. Σε αυτή την περίπτωση, αναφερόμαστε για \textbf{Distributed} αλγορίθμους\udot όπου κάθε node θα 
...
και προβλήματα με το scalability του συστήματος - καθώς προτείνεται για μικρότερα networks. 
