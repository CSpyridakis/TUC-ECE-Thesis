...
%----------------------------------------------------------------------
\subsection{Angle}\label{sec:Chapter2-1-2}
...
της \emph{anisotropy antenna}. Το pattern αυτού του τύπου κεραίας, παρουσιάζεται στο \emph{Figure} \ref{fig:Typical-anisotropic-antenna-pattern}
...
Εάν χρησιμοποιούμε \emph{directional antenna} στον receiver και μπορούμε είτε με μηχανικό είτε με ηλεκτρονικό τρόπο να στρέψουμε
...
είναι να χρησιμοποιήσουμε μία δεύτερη - μη κινούμενη - \emph{omnidirectional} κεραία και να κανονικοποιήσουμε τις μετρήσεις της directional με βάση τις   
...
Σε αυτήν την περίπτωση χρησιμοποιούμε \emph{omnidirectional} κεραίες και τον υπολογισμό της διαφοράς φάσης μπορεί να γίνει
...
\begin{table}[H]
    \caption{Distance/angle estimation techniques and position Computation summary \cite{advantages-disadvantages-toa-tdoa-aoa}}
...
υπόψιν\udot καθώς και αναφορά αλγόριθμων από την υπάρχουσα βιβλιογραφία, με τους οποίους μπορούμε να καλύψουμε αυτές τις
ανάγκες. 
...
\emph{v} μπορεί να επικοινωνήσει πίσω στο \emph{u}, τα οποία είναι διασκορπισμένα σε ένα δισδιάστατο τετραγωνικό
...
\begin{itemize}
	...
	\item $w(e) \le r$ με $e = \langle i,j\rangle$, να είναι το βάρος της κάθε ακμής και να χρησιμοποιείται για την έννοια της απόστασης
...
%----------------------------------------------------------------------
\subsection{One-hop vs Multi-hop}
Η πρώτη κατηγοριοποίηση στην οποία θα αναφερθούμε είναι - για αυτό το δίκτυο γράφου - προκειμένου να εκτιμήσουμε την θέση των nodes\udot 
από ποιους κόμβους τελικά θα αξιοποιήσουμε πληροφορία. Στους αλγόριθμους \textbf{One-hop} αξιοποιείται πληροφορία,
...
του τύπου τις εφαρμογές μας ενδιαφέρει η απόλυτη θέση στον χώρου οπότε θέλουμε μεθοδολογίες για \textbf{Absolute Positioning}, ενώ 
...
Ξεκινώντας, πρέπει να αναφερθεί ότι είναι πολύ σημαντικό να είναι ξεκάθαρο το σημείο\udot στο οποίο
...
έπειτα θα ενημερώνονται μέσω αυτού για την θέση τους - οπότε αναφερόμαστε για Anchor-free τεχνικές. Αντίθετα άλλη εναλλακτική 
είναι να γίνει τοπικά η επεξεργασία. Σε αυτή την περίπτωση, αναφερόμαστε για \textbf{Decentralized} ή \textbf{Distributed} αλγορίθμους\udot όπου κάθε node θα 
...
\begin{algorithm}[H]
	\caption{Multi Dimensional Scaling-Mobile Assisted Programming}\label{alg:MDS-MAP}
	\begin{algorithmic}[1]
		\Procedure{MDS-MAP}{}
...
\begin{algorithm}[H]
	\caption{Semi Definite Programming}\label{alg:SDP}
	\begin{algorithmic}[1]
		\Procedure{SDP}{}
...
\begin{algorithm}[H]
	\caption{Localization Based on Simulated Annealing}\label{alg:LBSA}
	\begin{algorithmic}[1]
		\Procedure{LBSA}{}
...
πιθανόν να χρειαστούμε ακριβότερα nodes - λόγο των έξτρα components ή του πιο sophisticated hardware.  
...
\begin{algorithm}[H]
	\caption{Approximate Point in Triangle}\label{alg:APIT}
	\begin{algorithmic}[1]
		\Procedure{APIT}{}
...
\begin{algorithm}[H]
	\caption{Distance Vector-Hop}\label{alg:DV-Hop}
	\begin{algorithmic}[1]
		\Procedure{DV-Hop}{}
