...
έπειτα θα ενημερώνονται μέσω αυτού για την θέση τους - οπότε αναφερόμαστε για \textbf{Centralized} τεχνικές. Αντίθετα άλλη εναλλακτική 
...
\begin{table}[H]
    \caption{Comparison of Centralized vs Distributed \cite{localization-algorithms-comparizon-tables}}
...
\begin{algorithm}[H]
	\caption{Multi Dimensional Scaling-Mobile Assisted Programming \cite{localization-algorithms}}\label{alg:MDS-MAP}
...
\begin{table}[H]
    \caption{Comparison of Range-based vs Range-free \cite{localization-algorithms-comparizon-tables}}
...
\begin{algorithm}[H]
	\caption{Approximate Point in Triangle \cite{localization-algorithms}}\label{alg:APIT}
	\begin{algorithmic}[1]
...
\begin{algorithm}[H]
	\caption{Distance Vector-Hop \cite{dv-hop-algo} \cite{dv-hop-algo2} \cite{dv-hop-algo3}}\label{alg:DV-Hop}
	\begin{algorithmic}[1]
		\Statex{{\bf procedure} DV-Hop}{}
			...
			\For{all free nodes received all messages from neighbors}
				\State Neighbor nodes record the node ID, the coordinate values, and  the smallest  
				\newline\hspace*{1.5em}hop. Then forward packet after incrementing hop by plus 1.
      		\EndFor

			\For{all  nodes}
			\State Estimate average hop distance $HopSize_i = \frac{\sum{\sqrt{(x_i-x_j)^2+(y_i-y_j)^2}}}{\sum{hop_{i,j}}}, i\neq j$
			\EndFor
			\State
